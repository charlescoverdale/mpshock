...
Those series live today as XLS and CSV files scattered across individual authors' personal pages, Federal Reserve research data pages, and GitHub mirrors, with inconsistent column names, date formats, and update cadences. `mpshock` centralises them as tidy data frames with provenance, loaded in one call, no network required.
